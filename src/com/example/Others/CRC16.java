...
public class CRC16 {
	...
	public static void CheckCRC16(byte[] value,int len,byte[] result)
	{
		...
		byte tCRCLo = (byte) 0xFF ; //低CRC字节初始化
	...
	/*
	 * 计算CRC16校验码，返回32位的16进制字符串
	 */
	public static String CheckCRC16(byte[] value,int len)
	{
		byte[] re = new byte[2];
		CheckCRC16(value,len,re);
		String ss = String.format("%1$02x", BTI(re[1]));//Integer.toString((re[0]+256)%256,16);
		ss += String.format("%1$02x", BTI(re[0]));//Integer.toString((re[1]+256)%256, 16);//.toBinaryString(re[1]);
		ss += "0000";
		return ss;
	}
