...
public class CellInfoManager {
	...
	private boolean valid = false; // ??不懂这个值什么意思
	...
	public JSONArray cellTowers() {
		...
		if (aryCell == null || aryCell.length < 2) {
			aryCell = new int[2];
			aryCell[0] = cid;
			aryCell[1] = -60;// ?不懂为啥新增一个-60的变量，而下面的循环又 i+=2 ???
		}// [2808634,-60]
		for (int i = 0; i < aryCell.length; i += 2) {
			try {
				...
				jsonobject.put("location_area_code",// 40986 - -1
						lat);
				jsonobject.put("mobile_country_code",// 460 - 中国
						mcc);
				jsonobject.put("mobile_network_code",// 1
						mnc);
		...
		if (isCdma()) {
			...
			CdmaCellLocation location = (CdmaCellLocation) tel
					.getCellLocation();
			int cellIDs = location.getBaseStationId(); // 3139
			int networkID = location.getSystemId(); // 2
			int areaCode = location.getNetworkId();
			try {
				...
				JSONObject jsonobject = new JSONObject();
				jsonobject.put("cell_id", cellIDs);// this.bid
				jsonobject.put("location_area_code", areaCode);// this.nid
				jsonobject.put("mobile_country_code", tel.getNetworkOperator()
						.substring(0, 3));
				jsonobject.put("mobile_network_code", networkID);// this.sid
				jsonobject.put("age", 0);
				jsonarray.put(jsonobject);
			} catch (Exception ex) {
				ex.printStackTrace();
	...
	public void update() {
		...
		if (strNetworkOperator.length() == 5) {
			// 460 国家编号 [中国]
			this.mcc = Integer.parseInt(strNetworkOperator.substring(0, 3));
			// 网络类型
			this.mnc = Integer.parseInt(strNetworkOperator.substring(3, 5));
		}
		/**
		 * PHONE_TYPE_NONE 0 PHONE_TYPE_GSM 1 PHONE_TYPE_CDMA 2
		 */
		...
		if (nPhoneType == 1 && cellLocation instanceof GsmCellLocation) {
			...
			try {
				// TODO valid 只有在此处被赋值为true 不懂这是为什吗？？？？
				this.valid = true;
				this.isCdma = true;
