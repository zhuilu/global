...
public class BcdUtils {
	...
	// 二进制字节转换为可打印字符串，例如 {0x01,0x02} ==》 "0102"
	...
	// 可打印字符串转换为二进制字节数组，例如 "0102"==>{0x01,0x02}
	public static byte[] stringToByteArray(String str) {
		String standStr = standardizationHexString(str + " ");
		byte bin[] = new byte[standStr.length() / 2];

		for (int i = 0; i < bin.length; i++) {
			bin[i] = (byte) ((BcdUtils.charToByte(standStr.charAt(i * 2)) << 4) + BcdUtils
					.charToByte(standStr.charAt(i * 2 + 1)));
		}
		return bin;
	}
	...
	public static String byteToString(byte b) {
		...
		buff.append(BcdUtils.byteToChar((b >> 4) & 0x0F)).append(
				BcdUtils.byteToChar(b & 0x0F));

		return buff.toString();
	}

	public static byte stringToByte(String str) {
		return (byte) (BcdUtils.charToByte(str.charAt(0)) * 0x10 + BcdUtils
				.charToByte(str.charAt(1)));
	}
	...
	public static Date bcdToDate(byte[] bcdArray, String format,
		...
		if (yyyyidx != -1) {
			calendar.set(
					...
					(int) BcdUtils.bcdToInt(bcdArray, beginPosition + yyyyidx
							/ 2, 2));
		} else {
			...
			if (yyindex != -1) {
				calendar.set(
						GregorianCalendar.YEAR,
						(calendar.get(GregorianCalendar.YEAR) / 100)
								* 100
								+ BcdUtils.bcdToInt(bcdArray[beginPosition
										+ yyindex / 2]));
			}
		}
	...
	public static byte[] dateToBcd(Date date, String format) {
		...
		int yyyyidx = uformat.indexOf("YYYY");
		if (yyyyidx != -1) {
			byte[] years = BcdUtils.intTobcd(
					calendar.get(GregorianCalendar.YEAR), 2);
			result[yyyyidx / 2] = years[0];
			result[yyyyidx / 2 + 1] = years[1];
		} else {
			int yyindex = uformat.indexOf("YY");
			if (yyindex != -1) {
				result[yyindex / 2] = BcdUtils.intToBcd((byte) (calendar
						.get(GregorianCalendar.YEAR) % 100));
			}
		}

		int mmindex = uformat.indexOf("MM");
		if (mmindex != -1) {
			result[mmindex / 2] = BcdUtils.intToBcd((byte) (calendar
					.get(GregorianCalendar.MONTH) + 1));
		}

		int ddindex = uformat.indexOf("DD");
		if (ddindex != -1) {
			result[ddindex / 2] = BcdUtils.intToBcd((byte) (calendar
					.get(GregorianCalendar.DAY_OF_MONTH)));
		}

		int hhindex = uformat.indexOf("HH");
		if (hhindex != -1) {
			result[hhindex / 2] = BcdUtils.intToBcd((byte) (calendar
					.get(GregorianCalendar.HOUR_OF_DAY)));
		}

		int miindex = uformat.indexOf("MI");
		if (miindex != -1) {
			result[miindex / 2] = BcdUtils.intToBcd((byte) (calendar
					.get(GregorianCalendar.MINUTE)));
		}

		int ssindex = uformat.indexOf("SS");
		if (ssindex != -1) {
			result[ssindex / 2] = BcdUtils.intToBcd((byte) (calendar
	...
	public static byte[] IpPortStringToBytes(String ipportString) {
		String[] strs = ipportString.substring(0, ipportString.indexOf(":"))
				.split("[.]");
	...
	private static byte charToByte(char ch) {
		return ((ch >= '0') && (ch <= '9')) ? (byte) (ch - '0')
				: (byte) (Character.toUpperCase(ch) - 'A' + 10);
	}
	...
	private static String standardizationHexString(String str) {
		...
		while (firstTokenPosition != -1) {
			...
			firstTokenPosition = findFirstTokenPosition(str,
					endTokenPosition + 1);
		}
